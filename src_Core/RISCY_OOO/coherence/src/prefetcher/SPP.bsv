// SPP.bsv -- Signature Path Prefetcher (Kim/Pugsley/Gratz/Reddy/Wilkerson/Chishti MICRO '16)
//
// Port of Karlis Susters's SPP implementation from the CHERI-Toooba
// `ldh35-fpga-cap_chaser` branch (SignaturePathPrefetcher.bsv, 2023,
// MIT license). Submodules (Divider, PrefetchCalculator, SignatureTable,
// PatternTable, PrefetchFilter) are kept verbatim from ldh35. The
// top-level module `mkSPP` is rewritten to expose the main-branch
// CacheLinePrefetcher#(reqT) interface and drops PERFORMANCE_MONITORING
// perf-event plumbing.
//
// Mechanism summary (paper §III):
//   - Signature Table (ST): per-page state, indexed by physical page
//     tag. Compresses the last 4 deltas in a 4KB page into a 12-bit
//     signature via sig' = (sig << 3) XOR delta.
//   - Pattern Table (PT): signature-indexed, up to 4 (delta, count)
//     predictions per entry with saturating counters (Csig, Cdelta).
//   - Prefetch Calculator: recursive lookahead.
//     Pd = alpha * (Cdelta / Csig) * P(d-1); issue while Pd >= Tp.
//   - Prefetch Filter: 1024-entry dedup + tracks global accuracy
//     (Cuseful/Ctotal => alpha).
// Page-boundary learning (GHR) from the paper is NOT present in this
// port - ldh35 did not implement it.
//
// Original license (ldh35 branch):
//   Copyright (c) 2023 Karlis Susters -- MIT-style permissive
//   license. See SignaturePathPrefetcher.bsv on
//   origin/ldh35-fpga-cap_chaser for full text.
...
// Top-level: mkSPP
// -----------------------------------------------------------------
// Adapts the ldh35 submodules to the current tree's CacheLinePrefetcher
// interface. Training fires on every L1 access via reportAccess; the
// SPP learning flow then drives addrToPrefetch, which getNextPrefetchAddr
// drains into the main L1Bank prefetch-issue pipeline.
//
// Fixed-sized instantiation (matches paper's canonical config):
//   ST  = 64 sets x 4 ways (256 entries), 11008 bits
//   PT  = 512 entries                    , 24576 bits
//   PF  = 1024 entries                   ,  8192 bits
//   Total ~ 5.37 KB per paper Table II.
// Threshold Tp = 0x20 (in 7-bit fixed-point; ~0.25 in floating point).
...
provisos (
    ...
    // Paper's canonical SPP configuration.
    Prob prefetchThreshold = 7'b0100000;   // ~0.25
    Bool useFilter = True;
    String divTableFile = "";              // unused; table inlined via include

    PrefetchCalculator#(8, 8) calculator <- mkPrefetchCalculator(prefetchThreshold, divTableFile);
    SignatureTable#(4, 64, 4) st <- mkSignatureTable;   // 64 sets * 4 ways = 256
    ...
    // Translated prefetch output (paddr + vpn) waiting for L1 consumption.
    Fifo#(8, PendingPrefetch) nextCandidateBuffer <- mkOverflowBypassFifo;

    // Bookkeeping for TLB translation of prefetch candidates. SPP was
    // designed for physical-address-only operation; since Toooba's
    // prefetcher interface issues vaddrs through a shared TLB path,
    // we run each SPP-generated lineAddr through the TLB and drop
    // translations that miss/except.
    ...
    // SPP internal pipeline wiring (unchanged from ldh35).
    rule ptlFromStToPt;
    ...
    rule handleTlbResp(tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            if (verbose) $display("%t SPP: enqueue prefetch paddr %h", $time, resp.paddr);
        end else begin
            if (verbose) $display("%t SPP: dropped prefetch lineAddr %h (exc=%b mainMem=%b)",
                $time, lineAddr, resp.haveException, paddrInMainMem);
        end
    endrule

    // -------------- CacheLinePrefetcher methods ----------------
    method Action reportAccess(Addr addr, Bit#(16) pcHash, HitOrMiss hitMiss,
                               Line line, Vpn reqVpn, MemOp op, Bool isPrefetch);
        // SPP trains on every L1 demand access (both hits and misses,
        // both Ld and St). Feed the ST + filter. Drop prefetch-op
        // accesses so we don't double-count.
        if (!isPrefetch) begin
