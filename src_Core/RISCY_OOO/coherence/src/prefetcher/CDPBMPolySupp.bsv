...
typedef struct { LineAddr lineAddr; } BMPolySuppFilterEntryT deriving (Bits, FShow, Eq);

typedef struct {
    Addr paddr;
    Addr vaddr;
    Bool routeLLC;
} NextCandPolySuppT deriving (Bits, FShow, Eq);

module mkCDPBMPolySupp#(
...
provisos (
    ...
    Fifo#(16, NextCandPolySuppT) nextCandidateBuffer <- mkOverflowBypassFifo;
    ...
    RWBramCore#(Bit#(10), Maybe#(BMPolySuppFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        // BMPolySupp: SUPPRESS multi-match lines entirely (vs POLY which
        // routes them to LLC). Multi-match means we can't tell which pointer
        // is real — rather than pre-warm LLC with possibly-wrong data, skip.
        // Only single-match lines (clean single-pointer detection) issue.
        ...
        if (!suppress) begin
            for (Integer i = 0; i < 8; i = i + 1) begin
                ...
                if (candUpper == missUpper) begin
                    candFIFO.enqS[enqIdx].enq(tuple2(x.line[i], False));  // L1 route
                    $display("%t AlexLog: CDP BMPolySupp candidate offset: %d candVaddr: %h matchCount: %d L1",
                        cur_cycle, i, x.line[i], matchCount);
                    enqIdx = enqIdx + 1;
                end
            end
        end else begin
            $display("%t AlexLog: CDP BMPolySupp SUPPRESS matchCount: %d (multi-match)",
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BMPolySupp dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BMPolySupp filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMPolySuppFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandPolySuppT{paddr: paddr, vaddr: candVaddr, routeLLC: routeLLC});
            $display("%t AlexLog: CDP BMPolySupp filter MISS, issuing prefetch lineAddr %h routeLLC: %b", cur_cycle, lineAddr, routeLLC);
        end
    endrule
