...
typedef struct { LineAddr lineAddr; } PureStrideFilterEntryT deriving (Bits, FShow, Eq);

module mkCDPPureStride#(
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(PureStrideFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        l1ToCDP.deq;
        // PureStride: bit-matching DELIBERATELY DISABLED. Emit only stride
        // prefetches (degree 2 on miss + degree 2 on hit in reportAccess).
        // Everything else (dedup filter, TLB path, paddr range gate) is
        // unchanged from BM-EARLY3, so this is the exact bit-match ablation.
        Addr baseLine = getReqAddr(x.req) & 'hFFFF_FFFF_FFFF_FFC0;
        candFIFO.enqS[0].enq(baseLine + 64);
        candFIFO.enqS[1].enq(baseLine + 128);
        $display("%t AlexLog: CDP PureStride miss-stride +1: %h +2: %h",
            cur_cycle, baseLine + 64, baseLine + 128);
    endrule
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP PureStride dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP PureStride filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(PureStrideFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandT{paddr: paddr, vaddr: candVaddr, isNeighbourLine: False});
            $display("%t AlexLog: CDP PureStride filter MISS, issuing prefetch lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
    ...
    method Action reportAccess(Addr addr, Bit#(16) pcHash, HitOrMiss hitMiss, Line line, Vpn reqVpn, MemOp op, Bool isPrefetch);
        ...
        if (filterInited && tlbReqFreeQInited && hitMiss == HIT && op == Ld && !isPrefetch) begin
            ...
            candFIFO.enqS[0].enq(baseLine + 64);
            candFIFO.enqS[1].enq(baseLine + 128);
            $display("%t AlexLog: CDP PureStride hit-stride line+1: %h line+2: %h (from addr %h)",
