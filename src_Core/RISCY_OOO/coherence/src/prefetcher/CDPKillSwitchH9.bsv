// CDPKillSwitchH9.bsv — H7 + D-style path-history signature.
//
// Mechanism: maintain a 16-bit rolling path signature updated on every
// demand load drained from incomingQ:
//
//   scanSig = getPcHash(req) ^ pathSig           // read BEFORE update
//   pathSig <= (pathSig << 4) ^ getPcHash(req)   // shift-XOR, <<4 window
//
...
//
// pathSig is READ at point of use (deqCacheLines, ttAccess TT write,
// drainIncomingEvents EvDemandLdHit); it is WRITTEN in
// drainIncomingEvents on EvDemandLdMiss / EvDemandLdHit branches only,
// so no new method-guard path into L1. Signature lag between drain and
// downstream rules is O(1 cycle) in the common case.
//
// Motivation: path context (recent PC sequence) is a better predictor
// of next access than raw PC alone, especially for recursive (treeadd)
// or data-dependent-branching (patricia) workloads where the same PC
// is reached via different histories. Previous path-history variants
// (D, D2 in the variant catalog) pre-dated CapPtr §3.4.2 + overflow-
// bypass fixes and couldn't be fairly evaluated. H9 re-tries D's
// shift-XOR signature on the clean H7 pipeline.
//
...
module mkCDPStatefulRelativeKillSwitchH9#(
...
provisos (
    ...
    // D-style path signature. Updated in drainIncomingEvents on demand-load
    // events only (no method-body writes, so no back-pressure leak into L1).
    // Read at point of use in deqCacheLines / ttAccess / drainIncomingEvents
    // EvDemandLdHit to form scanSig = pcHash ^ pathSig.
    ...
    rule deqCacheLines;
        ...
        if (getReqOp(x.req) == Ld) begin
            ...
            Bit#(16) scanSigMiss = getPcHash(x.req) ^ pathSig;
    ...
    rule ttAccess(inited);
        ...
        if (respQ.isTrainingLookup) begin
            ...
            Bit#(16) scanSigScan = getPcHash(respQ.req) ^ pathSig;
    ...
    rule drainIncomingEvents;
        ...
        case (branch)
            EvDemandLdMiss: begin
                // Path-signature update: shift-XOR the current demand load's
                // pcHash into pathSig. Downstream deqCacheLines will re-read
                // pathSig to form scanSig at its own cycle (1-2 cycle lag).
            ...
            EvDemandLdHit: begin
                ...
                // Capture scanSig BEFORE updating pathSig.
                Bit#(16) scanSigHit = getPcHash(req) ^ pathSig;
                pathSig <= (pathSig << 4) ^ getPcHash(req);
