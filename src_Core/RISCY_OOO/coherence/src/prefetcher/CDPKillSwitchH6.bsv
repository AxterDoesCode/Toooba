...
module mkCDPStatefulRelativeKillSwitchH6#(
...
provisos (
    ...
    // H6: enlarge the staging FIFO from 32 to 256 so drainIncomingEvents has
    // more slack in compute-light phases where L1 hit rate exceeds CDP's
    // downstream drain throughput. Confirmed via instrumentation that the
    // 32-entry staging FIFO fills to 32 for 9.9% of em3d cycles (20-51% in
    // compute-light phase); this stalls reportIncomingCacheLine which blocks
    // pipelineResp_cRq in L1Bank. Larger FIFO should buffer over the
    // transient compute-light bursts.
    FIFO#(Tuple3#(CDPIncomingBranchT, reqT, Line)) incomingQ <- mkSizedFIFO(256);
