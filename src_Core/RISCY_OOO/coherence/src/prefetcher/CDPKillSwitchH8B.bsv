// Variant H8B: 4-bit counters + halving-on-saturation.
//
// This variant isolates the *halving* knob at the original 4-bit
// saturation point (H7's width), letting us tell apart "widening
// caused the H8 regression" from "halving caused the H8 regression".
//
// Relative to H7:
//  - Same struct refactor as H8 (PendingTlbReqT).
//  - 4-bit usefulCount / uselessCount (same as H7, max 15).
//  - Halving-on-saturation applied on every would-saturate AccUseful
//    and AccUseless, same mechanics as H8.
//
// Expected: on the health bench, this should regress MORE than H8
// (6-bit + halving), because after halving at 4-bit the +3 useless
// amplification is a bigger fraction of the halved headroom
// (uf:7, us:7 → us 10 > uf 7) vs the 6-bit case (uf:31, us:34).
//
...
module mkCDPStatefulRelativeKillSwitchH8B#(
        ...
        case (tag) matches
            ...
            tagged AccUseful .pcHash: begin
                if (rdResp matches tagged Valid .e &&& e.pcHash == pcHash) begin
                    ...
                    Bit#(4) baseUf = sat ? (e.usefulCount >> 1) : e.usefulCount;
                    Bit#(4) baseUs = sat ? (e.uselessCount >> 1) : e.uselessCount;
                    Bit#(4) newUf = baseUf + 1;   // guaranteed unsaturated after halving
                    pcTable.wrReq(pctIdx, Valid(PCTableAccEntryT{
            ...
            tagged AccUseless .pcHash: begin
                if (rdResp matches tagged Valid .e &&& e.pcHash == pcHash) begin
                    ...
                    Bit#(5) amplifiedRaw = zeroExtend(e.uselessCount) + 3;
                    Bool sat = (amplifiedRaw >= 15);
                    Bit#(4) baseUf = sat ? (e.usefulCount >> 1) : e.usefulCount;
                    Bit#(4) baseUs = sat ? (e.uselessCount >> 1) : e.uselessCount;
                    Bit#(5) amplifiedAdj = zeroExtend(baseUs) + 3;
                    Bit#(4) newUs = (amplifiedAdj >= 15) ? 15 : truncate(amplifiedAdj);
                    pcTable.wrReq(pctIdx, Valid(PCTableAccEntryT{
                        pcHash: e.pcHash, conf: e.conf,
