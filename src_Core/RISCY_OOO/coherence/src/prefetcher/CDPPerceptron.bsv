// CDPPerceptron.bsv -- Variant M: PPF-style per-(PC, relOff) perceptron
// filter on CDP decisions. Base: Variant J (unbiased 4096-entry attribution
// table, reused) with the per-PC kill-switch gate removed and replaced by a
// 256-entry signed-weight perceptron indexed by hash(pcHash) XOR relOff+7.
//
// On prefetch decision: issue iff perceptron weight >= killThreshold
// (typical: 0 — issue when weight is non-negative). On attribution events:
// - useful-hit (line was demand-touched): weight[idx] += 1 (saturate)
// - useless-evict (line evicted without demand): weight[idx] -= 1 (saturate)
// Weights are signed Int#(4), range -8..+7.
//
// Hypothesis: CDP's existing conf table monotonically trains UP (+decay).
// It can't distinguish "this (PC, relOff) pair fired confidently but was
// never used" from "fired confidently AND was used". The perceptron's
// ability to go NEGATIVE on persistent uselessness should suppress the
// health/treeadd false-positive prefetches that kill-switch variants
// (H4/J/J2) couldn't separate.
//
...
} PCTablePPFEntryT deriving (Bits, FShow, Eq);
...
typedef struct {
    ...
    Bool routeLLC;
} NextCandPPFT deriving (Bits, FShow, Eq);

// Dedup filter entry: minimal.
typedef struct { LineAddr lineAddr; } PPFFilterEntryT deriving (Bits, FShow, Eq);
...
typedef struct {
    ...
    Bool         punishable;
} PPFAttribEntryT deriving (Bits, FShow, Eq);
...
typedef union tagged {
    ...
    void                             Decay;
} PCTableRdPPFTagT deriving (Bits, FShow);

module mkCDPStatefulRelativePPF#(
...
provisos (
    ...
    RWBramCore#(pcTableIdxT, Maybe#(PCTablePPFEntryT)) pcTable <- mkRWBramCoreForwarded();
    // Dedup filter -- 1024 entries (Bit#(10)) as before.
    RWBramCore#(Bit#(10), Maybe#(PPFFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    // Attribution table -- 4096 entries (Bit#(12)), independent of the filter.
    RWBramCore#(Bit#(12), Maybe#(PPFAttribEntryT)) attribTable <- mkRWBramCoreForwarded();
    ...
    Fifo#(16, NextCandPPFT) nextCandidateBuffer <- mkOverflowBypassFifo;
    FIFO#(Tuple2#(pcTableIdxT, PCTableRdPPFTagT)) pcTableRdReqFIFO <- mkSizedFIFO(64);
    FIFO#(Tuple2#(pcTableIdxT, PCTableRdPPFTagT)) pcTableRdTagQ    <- mkFIFO;
    FIFO#(Tuple4#(Bit#(10), NextCandPPFT, Bit#(16), RelLineOffset)) filterPendingQ <- mkFIFO;
    ...
    rule pcTableResp(inited);
        ...
        case (tag) matches
            tagged Training {.pcHash, .relOffset}: begin
                ...
                pcTable.wrReq(pctIdx, Valid(PCTablePPFEntryT{
                    pcHash: pcHash,
                    conf: update(curConf, vecIdx, newVal)}));
                $display("%t AlexLog: CDP PPF PC table updated, idx: %d pcHash: %h relOffset: %d conf: %d -> %d",
            ...
            tagged PrefetchIssue {.addr, .line, .reqVpn}: begin
                if (rdResp matches tagged Valid .entry) begin
                    ...
                    if (foundHighConf) begin
                        Bool okToIssue = perceptronVote(entry.pcHash, bestRelOffset);
                        Int#(4) w = perceptron[perceptronIdx(entry.pcHash, bestRelOffset)];
                        $display("%t AlexLog: CDP PPF prefetch decision: pcHash %h relOffset %d weight %d %s",
    ...
    rule processTlbResp;
        ...
        if (!resp.haveException) begin
            NextCandPPFT cand = NextCandPPFT{
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP PPF filter HIT: dropped duplicate prefetch for lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(PPFFilterEntryT{ lineAddr: lineAddr }));
            Bit#(12) attribIdx = hash(lineAddr);
            attribTable.wrReq(attribIdx, Valid(PPFAttribEntryT{
                lineAddr: lineAddr, pcHash: pcH, relOffset: relOff, punishable: True }));
            nextCandidateBuffer.enq(cand);
            $display("%t AlexLog: CDP PPF filter MISS: issuing prefetch for lineAddr %h pcHash %h relOff %d",
    ...
    rule finishEviction(attribInited);
        ...
        if (rdResp matches tagged Valid .ae &&& ae.lineAddr == lineAddr) begin
            if (ae.punishable) begin
                ...
                $display("%t AlexLog: CDP PPF useless bump: lineAddr %h pcHash %h relOff %d w %d -> %d",
    ...
    rule finishUsefulLookup(attribInited);
        ...
        if (rdResp matches tagged Valid .ae &&& ae.lineAddr == lineAddr &&& ae.punishable) begin
            ...
            Int#(4) wNew = satAdd(w, 1);
            perceptron[pIdx] <= wNew;
            $display("%t AlexLog: CDP PPF useful bump: lineAddr %h pcHash %h relOff %d w %d -> %d",
                cur_cycle, lineAddr, ae.pcHash, ae.relOffset, w, wNew);
        end
    endrule
    ...
    method ActionValue#(PendingPrefetch) getNextPrefetchAddr;
        ...
        $display("%t AlexLog: CDP PPF prefetch issued. lineAddr: %h",
