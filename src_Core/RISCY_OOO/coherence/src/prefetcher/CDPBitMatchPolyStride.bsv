...
typedef struct { LineAddr lineAddr; } BMPolyStrideFilterEntryT deriving (Bits, FShow, Eq);

typedef struct {
    Addr paddr;
    Addr vaddr;
    Bool routeLLC;
} NextCandPolyStrideT deriving (Bits, FShow, Eq);

module mkCDPBitMatchPolyStride#(
...
provisos (
    ...
    Fifo#(16, NextCandPolyStrideT) nextCandidateBuffer <- mkOverflowBypassFifo;
    ...
    RWBramCore#(Bit#(10), Maybe#(BMPolyStrideFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        for (Integer i = 0; i < 8; i = i + 1) begin
            ...
            if (candUpper == missUpper) begin
                ...
                $display("%t AlexLog: CDP BitMatchPolyStride candidate offset: %d candVaddr: %h matchCount: %d routeLLC: %b",
        ...
        if (enqIdx < 8) begin
            ...
            $display("%t AlexLog: CDP BitMatchPolyStride stride candidate candVaddr: %h",
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BitMatchPolyStride dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BitMatchPolyStride filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMPolyStrideFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandPolyStrideT{paddr: paddr, vaddr: candVaddr, routeLLC: routeLLC});
            $display("%t AlexLog: CDP BitMatchPolyStride filter MISS, issuing prefetch lineAddr %h routeLLC: %b", cur_cycle, lineAddr, routeLLC);
        end
    endrule
