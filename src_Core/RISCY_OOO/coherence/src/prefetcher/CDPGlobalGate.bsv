// CDPGlobalGate.bsv -- Variant YY: GLOBAL trust-score gate.
// Base: Variant O (utility-gated conf, attribTable, pcTable pipelined UtilBump).
// Add: a signed global score register that tracks aggregate useful - useless
// attribution across ALL PCs. Decision logic:
//   - score >= 0 -> gate open -> issue normally
//   - score <  0 -> gate closed -> suppress decision, UNLESS 1/8 LFSR bleed
//     fires, in which case still issue (to keep gathering training data)
// Decay: every 4096 cycles, halve the score toward 0 (fresh behaviour wins).
//
// Motivation: oracle analysis of our bench suite showed that turning CDP
// OFF on 4/5 benchmarks (and ON only for patricia) would yield +13.3%
// geomean vs decay16 baseline — a 3x improvement over L2's +4.14% current
// best. The per-PC filters we have (H kill-switch, M/N perceptron) only
// squeeze <0.1% above the baseline decay256 line. The missing piece is
// SYSTEM-LEVEL auto-disable: when the whole-suite accuracy tanks, stop
// issuing, not just "filter this PC".
//
// Why global rather than per-PC: benchmark kernels have phase-coherent
// behaviour. Within a kernel, MOST PCs share a fate (all pollute, or
// all contribute). Per-PC filters are too slow to respond to whole-
// kernel pollution; a global counter integrates the signal fast.
//
//
...
} PCTableGlobalGateEntryT deriving (Bits, FShow, Eq);
...
typedef struct {
    ...
    Bool routeLLC;
} NextCandGlobalGateT deriving (Bits, FShow, Eq);

// Dedup filter entry: minimal.
typedef struct { LineAddr lineAddr; } GlobalGateFilterEntryT deriving (Bits, FShow, Eq);
...
typedef struct {
    ...
    Bool         punishable;
} GlobalGateAttribEntryT deriving (Bits, FShow, Eq);
...
} PCTableRdGlobalGateTagT deriving (Bits, FShow);

module mkCDPStatefulRelativeGlobalGate#(
...
provisos (
    ...
    RWBramCore#(pcTableIdxT, Maybe#(PCTableGlobalGateEntryT)) pcTable <- mkRWBramCoreForwarded();
    // Dedup filter -- 1024 entries (Bit#(10)) as before.
    RWBramCore#(Bit#(10), Maybe#(GlobalGateFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    // Attribution table -- 4096 entries (Bit#(12)), independent of the filter.
    RWBramCore#(Bit#(12), Maybe#(GlobalGateAttribEntryT)) attribTable <- mkRWBramCoreForwarded();

    Fifo#(16, NextCandGlobalGateT) nextCandidateBuffer <- mkOverflowBypassFifo;
    FIFO#(Tuple2#(pcTableIdxT, PCTableRdGlobalGateTagT)) pcTableRdReqFIFO <- mkSizedFIFO(64);
    FIFO#(Tuple2#(pcTableIdxT, PCTableRdGlobalGateTagT)) pcTableRdTagQ    <- mkFIFO;
    FIFO#(Tuple4#(Bit#(10), NextCandGlobalGateT, Bit#(16), RelLineOffset)) filterPendingQ <- mkFIFO;
    ...
    // Variant YY: global trust score. Tracks recent useful-useless attribution
    // balance across ALL PCs. Gate: issue iff score >= 0. Bleed-through: 1/8
    // of suppressed decisions still issue (provides training data to re-adjust
    // score when kernel changes). Decay: every 4096 cycles score halves toward
    // 0 (freshness vs stability tradeoff).
    Reg#(Int#(9)) globalScore <- mkReg(0);
    Reg#(Bit#(32)) scoreDecayCounter <- mkReg(4096);
    LFSR#(Bit#(16)) bleedLfsr <- mkLFSR_16;
    Reg#(Bool) bleedLfsrInited <- mkReg(False);
    ...
    rule pcTableResp(inited);
        ...
        case (tag) matches
            tagged Training {.pcHash, .relOffset}: begin
                ...
                pcTable.wrReq(pctIdx, Valid(PCTableGlobalGateEntryT{
                    pcHash: pcHash,
                    conf: update(curConf, vecIdx, newVal)}));
                $display("%t AlexLog: CDP GlobalGate PC table trained (eligible), idx: %d pcHash: %h relOffset: %d conf: %d -> %d",
            ...
            tagged UtilBump {.pcHash, .relOffset, .isDecrement}: begin
                ...
                // Also update the GLOBAL score register.
                // Only act if entry matches pcHash (else the PC entry was
                // evicted/replaced and the bump is stale).
                Int#(9) globalDelta = isDecrement ? -1 : 1;
                Int#(10) newGlobalScore = extend(globalScore) + extend(globalDelta);
                Int#(9) clippedNewGlobal = (newGlobalScore > 255) ? 255
                                         : (newGlobalScore < -256) ? -256
                                         : truncate(newGlobalScore);
                globalScore <= clippedNewGlobal;
                if (rdResp matches tagged Valid .e &&& e.pcHash == pcHash) begin
                    ...
                    pcTable.wrReq(pctIdx, Valid(PCTableGlobalGateEntryT{
                        pcHash: e.pcHash,
                        conf: update(curConf, vecIdx, newVal)}));
                    $display("%t AlexLog: CDP GlobalGate %s bump, idx: %d pcHash: %h relOffset: %d conf: %d -> %d global %d",
                             cur_cycle, isDecrement ? "useless" : "useful",
                             pctIdx, pcHash, relOffset, curVal, newVal, clippedNewGlobal);
                end
            end
            tagged PrefetchIssue {.addr, .line, .reqVpn}: begin
                if (rdResp matches tagged Valid .entry) begin
                    ...
                    if (foundHighConf) begin
                        // Global trust gate: open if recent useful-useless balance is positive.
                        // When closed, still issue 1/8 of decisions (bleed-through) so that
                        // the score gets fresh attribution data if behaviour changes.
                        Bool gateOpen = globalScore >= 0;
                        Bool bleedThrough = (bleedLfsr.value[2:0] == 3'b000);
                        bleedLfsr.next;
                        Bool okToIssue = gateOpen || bleedThrough;
                        $display("%t AlexLog: CDP GlobalGate prefetch decision: pcHash %h relOffset %d score %d %s",
                            cur_cycle, entry.pcHash, bestRelOffset, globalScore,
                            okToIssue ? (gateOpen ? "ISSUE" : "BLEED") : "SUPPRESS");
                        if (okToIssue) begin
    ...
    rule processTlbResp;
        ...
        if (!resp.haveException) begin
            NextCandGlobalGateT cand = NextCandGlobalGateT{
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP GlobalGate filter HIT: dropped duplicate prefetch for lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(GlobalGateFilterEntryT{ lineAddr: lineAddr }));
            Bit#(12) attribIdx = hash(lineAddr);
            attribTable.wrReq(attribIdx, Valid(GlobalGateAttribEntryT{
                lineAddr: lineAddr, pcHash: pcH, relOffset: relOff, punishable: True }));
            nextCandidateBuffer.enq(cand);
            $display("%t AlexLog: CDP GlobalGate filter MISS: issuing prefetch for lineAddr %h pcHash %h relOff %d",
    ...
    rule finishEviction(attribInited);
        ...
        if (rdResp matches tagged Valid .ae &&& ae.lineAddr == lineAddr) begin
            if (ae.punishable) begin
                ...
                $display("%t AlexLog: CDP GlobalGate useless attrib: lineAddr %h pcHash %h relOff %d",
    ...
    rule finishUsefulLookup(attribInited);
        ...
        if (rdResp matches tagged Valid .ae &&& ae.lineAddr == lineAddr &&& ae.punishable) begin
            ...
            $display("%t AlexLog: CDP GlobalGate useful attrib: lineAddr %h pcHash %h relOff %d",
                cur_cycle, lineAddr, ae.pcHash, ae.relOffset);
        end
    endrule
    ...
    rule initBleedLfsr(!bleedLfsrInited);
        bleedLfsr.seed('h3c5a);
        bleedLfsrInited <= True;
    endrule

    // Global-score decay: every 4096 cycles, halve toward 0 (stay in the
    // current regime but let recent behaviour dominate).
    rule tickScoreDecay(inited);
        if (scoreDecayCounter == 0) begin
            scoreDecayCounter <= 4096;
            Int#(9) halved = globalScore >> 1;
            globalScore <= halved;
        end else begin
            scoreDecayCounter <= scoreDecayCounter - 1;
        end
    endrule
    ...
    method ActionValue#(PendingPrefetch) getNextPrefetchAddr;
        ...
        $display("%t AlexLog: CDP GlobalGate prefetch issued. lineAddr: %h",
