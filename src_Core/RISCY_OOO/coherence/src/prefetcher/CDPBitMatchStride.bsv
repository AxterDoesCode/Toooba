// CDPBitMatchStride.bsv -- Variant BM+STRIDE: BM's bit-matching augmented
// with an unconditional line+1 prefetch for every miss. Tests whether
// stride locality (sequential access pattern) stacks additively on top
// of Cooksey bit-matching pointer discovery. No learning layer — pure
// bit-match + pure next-line.
...
typedef struct { LineAddr lineAddr; } BMStrideFilterEntryT deriving (Bits, FShow, Eq);

module mkCDPBitMatchStride#(
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(BMStrideFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        for (Integer i = 0; i < 8; i = i + 1) begin
            ...
            if (candUpper == missUpper) begin
                ...
                $display("%t AlexLog: CDP BitMatchStride candidate offset: %d candVaddr: %h crossPage: %b",
        ...
        if (enqIdx < 8) begin
            ...
            $display("%t AlexLog: CDP BitMatchStride stride candidate candVaddr: %h",
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BitMatchStride dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BitMatchStride filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMStrideFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandT{paddr: paddr, vaddr: candVaddr, isNeighbourLine: False});
            $display("%t AlexLog: CDP BitMatchStride filter MISS, issuing prefetch lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
