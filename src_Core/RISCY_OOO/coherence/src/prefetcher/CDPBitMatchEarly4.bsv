...
typedef struct { LineAddr lineAddr; } BMEarly4FilterEntryT deriving (Bits, FShow, Eq);

module mkCDPBitMatchEarly4#(
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(BMEarly4FilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        for (Integer i = 0; i < 8; i = i + 1) begin
            ...
            if (candUpper == missUpper) begin
                ...
                $display("%t AlexLog: CDP BitMatchEarly4 candidate offset: %d candVaddr: %h crossPage: %b",
                    cur_cycle, i, x.line[i], getVpn(x.line[i]) != getReqVpn(x.req));
                enqIdx = enqIdx + 1;
            end
        end
        // Stride degree 3: emit line+1, line+2, line+3 on miss.
        Addr baseLine = getReqAddr(x.req) & 'hFFFF_FFFF_FFFF_FFC0;
        if (enqIdx < 8) begin
            candFIFO.enqS[enqIdx].enq(baseLine + 64);
            $display("%t AlexLog: CDP BitMatchEarly4 stride+1 candVaddr: %h", cur_cycle, baseLine + 64);
            enqIdx = enqIdx + 1;
        end
        if (enqIdx < 8) begin
            candFIFO.enqS[enqIdx].enq(baseLine + 128);
            $display("%t AlexLog: CDP BitMatchEarly4 stride+2 candVaddr: %h", cur_cycle, baseLine + 128);
            enqIdx = enqIdx + 1;
        end
        if (enqIdx < 8) begin
            candFIFO.enqS[enqIdx].enq(baseLine + 192);
            $display("%t AlexLog: CDP BitMatchEarly4 stride+3 candVaddr: %h", cur_cycle, baseLine + 192);
        end
    endrule
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly4 dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BitMatchEarly4 filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMEarly4FilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandT{paddr: paddr, vaddr: candVaddr, isNeighbourLine: False});
            $display("%t AlexLog: CDP BitMatchEarly4 filter MISS, issuing prefetch lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
    ...
    method Action reportAccess(Addr addr, Bit#(16) pcHash, HitOrMiss hitMiss, Line line, Vpn reqVpn, MemOp op, Bool isPrefetch);
        ...
        if (filterInited && tlbReqFreeQInited && hitMiss == HIT && op == Ld && !isPrefetch) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly4 hit-stride line+1: %h line+2: %h (from addr %h)",
