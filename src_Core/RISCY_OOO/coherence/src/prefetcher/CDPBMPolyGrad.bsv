...
typedef struct { LineAddr lineAddr; } BMPolyGradFilterEntryT deriving (Bits, FShow, Eq);

typedef struct {
    Addr paddr;
    Addr vaddr;
    Bool routeLLC;
} NextCandPolyGradT deriving (Bits, FShow, Eq);

module mkCDPBMPolyGrad#(
...
provisos (
    ...
    Fifo#(16, NextCandPolyGradT) nextCandidateBuffer <- mkOverflowBypassFifo;
    ...
    RWBramCore#(Bit#(10), Maybe#(BMPolyGradFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        // BMPolyGrad: GRADED routing by match count.
        //   matchCount == 1         -> L1  (clean single-pointer detection)
        //   matchCount in [2, 3]    -> LLC (some noise, but still likely useful
        //                                   to pre-warm LLC)
        //   matchCount >= 4         -> SUPPRESS (too noisy — likely false pos)
        Bool suppress = matchCount >= 4;
        ...
        if (!suppress) begin
            for (Integer i = 0; i < 8; i = i + 1) begin
                ...
                if (candUpper == missUpper) begin
                    ...
                    $display("%t AlexLog: CDP BMPolyGrad candidate offset: %d candVaddr: %h matchCount: %d routeLLC: %b",
                        cur_cycle, i, x.line[i], matchCount, routeLLC);
                    enqIdx = enqIdx + 1;
                end
            end
        end else begin
            $display("%t AlexLog: CDP BMPolyGrad SUPPRESS matchCount: %d (>=4)",
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BMPolyGrad dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BMPolyGrad filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMPolyGradFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandPolyGradT{paddr: paddr, vaddr: candVaddr, routeLLC: routeLLC});
            $display("%t AlexLog: CDP BMPolyGrad filter MISS, issuing prefetch lineAddr %h routeLLC: %b", cur_cycle, lineAddr, routeLLC);
        end
    endrule
