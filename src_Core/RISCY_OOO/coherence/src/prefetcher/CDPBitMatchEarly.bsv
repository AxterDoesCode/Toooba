...
typedef struct { LineAddr lineAddr; } BMEarlyFilterEntryT deriving (Bits, FShow, Eq);

module mkCDPBitMatchEarly#(
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(BMEarlyFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        Integer enqIdx = 0;
        // Bit-match pointer discovery (same as baseline BM).
        for (Integer i = 0; i < 8; i = i + 1) begin
            Bit#(matchBits) candUpper = truncateLSB(getVpn(x.line[i]));
            if (candUpper == missUpper) begin
                candFIFO.enqS[enqIdx].enq(x.line[i]);
                $display("%t AlexLog: CDP BitMatchEarly candidate offset: %d candVaddr: %h crossPage: %b",
                    cur_cycle, i, x.line[i], getVpn(x.line[i]) != getReqVpn(x.req));
                enqIdx = enqIdx + 1;
            end
        end
        // Additionally emit a line+1 stride prefetch for every miss — cheap
        // next-line locality. Only enqueue if we have a free candFIFO slot
        // (enqIdx < 8); dropping it otherwise is fine since we already
        // have plenty of pointer candidates in the queue anyway.
        if (enqIdx < 8) begin
            Addr nextLineVaddr = (getReqAddr(x.req) + 64) & 'hFFFF_FFFF_FFFF_FFC0;
            candFIFO.enqS[enqIdx].enq(nextLineVaddr);
            $display("%t AlexLog: CDP BitMatchEarly stride candidate candVaddr: %h",
                cur_cycle, nextLineVaddr);
        end
    endrule
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BitMatchEarly filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMEarlyFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandT{paddr: paddr, vaddr: candVaddr, isNeighbourLine: False});
            $display("%t AlexLog: CDP BitMatchEarly filter MISS, issuing prefetch lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
    ...
    method Action reportAccess(Addr addr, Bit#(16) pcHash, HitOrMiss hitMiss, Line line, Vpn reqVpn, MemOp op, Bool isPrefetch);
        // Early-stride: on every demand LD HIT, try to enqueue a line+1
        // prefetch candidate. Dedup filter drops recent duplicates.
        // Only enqueue if candFIFO slot[0] is free (conservative — don't
        // flood; let the miss-path get priority).
        if (filterInited && tlbReqFreeQInited && hitMiss == HIT && op == Ld && !isPrefetch) begin
            Addr nextLineVaddr = (addr + 64) & 'hFFFF_FFFF_FFFF_FFC0;
            candFIFO.enqS[0].enq(nextLineVaddr);
            $display("%t AlexLog: CDP BitMatchEarly hit-stride candVaddr: %h (from addr %h)",
