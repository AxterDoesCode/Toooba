// Behaviour identical to H7 (same kill-switch / ratio gate / overflow-bypass
// staging FIFO / 3x useless amplification). This file exists purely to
// apply readability refactors without disturbing H7 as the dissertation
// reference baseline.
//
// Refactors applied in this revision:
//  - The 6 parallel Vector#(LLCTlbReqNum, Reg#(T)) pending-TLB-state
//    registers (pendCandVaddr / pendIsNeighbourLine / pendCrossPage /
//    pendPcHash / pendPunishable / pendRouteLLC) are collapsed into a
//    single Vector#(LLCTlbReqNum, Reg#(PendingTlbReqT)), reducing the
//    per-field boilerplate at every write/read site in processTlbReq
//    and processTlbResp.
//  - usefulCount / uselessCount widened from 4 bits (max 15) to 6 bits
//    (max 63), pushing saturation out of the typical steady-state range
//    for long-running benches. Amplification arithmetic (AccUseless)
//    and decay arithmetic widened accordingly.
//  - Halving-on-saturation (SPP-style): when an increment on uf or us
//    would saturate, halve BOTH counters before applying the increment.
//    Preserves the uf:us ratio while freeing headroom, so the gate
//    stays discriminative even after long runs.
//
...
typedef struct {
    ...
    Bit#(6)          usefulCount;   // saturating at 63
    Bit#(6)          uselessCount;  // saturating at 63
...
// Per-outstanding-TLB-request bookkeeping. Holds every piece of state we
// need to reconstruct in processTlbResp. In H7 this was 6 separate
// parallel Vector#(LLCTlbReqNum, Reg#(T)) slots; folded here into a
// single Reg per id carrying the struct.
typedef struct {
    Addr     candVaddr;
    Bool     isNeighbourLine;
    Bool     crossPage;
    ...
    Bool     punishable;
    Bool     routeLLC;
} PendingTlbReqT deriving (Bits, FShow, Eq);
...
module mkCDPStatefulRelativeKillSwitchH8#(
...
provisos (
    ...
    // Per-TLB-id pending request bookkeeping (see PendingTlbReqT comment above).
    // routeLLC is the accuracy decision captured at prefetch-decision time —
    // flows through to getNextPrefetchAddr's routing choice.
    Vector#(LLCTlbReqNum, Reg#(PendingTlbReqT)) pendTlb <- replicateM(mkRegU);
    ...
    function Bool shouldIssue(Bit#(6) uf, Bit#(6) us);
        Bit#(6) threshold = fromInteger(valueOf(killThreshold));
    ...
    rule pcTableResp(inited);
        ...
        case (tag) matches
            tagged Training {.pcHash, .relOffset}: begin
                ...
                Bit#(6) curUseful = 0, curUseless = 0;
            ...
            tagged AccUseful .pcHash: begin
                if (rdResp matches tagged Valid .e &&& e.pcHash == pcHash) begin
                    // Halving-on-saturation (SPP-style). When uf would
                    // saturate, halve BOTH uf and us before the increment:
                    // preserves the ratio while freeing headroom so the
                    // gate stays discriminative as new events arrive.
                    Bool sat = (e.usefulCount == maxBound);
                    Bit#(6) baseUf = sat ? (e.usefulCount >> 1) : e.usefulCount;
                    Bit#(6) baseUs = sat ? (e.uselessCount >> 1) : e.uselessCount;
                    Bit#(6) newUf = baseUf + 1;   // guaranteed unsaturated after halving
                    pcTable.wrReq(pctIdx, Valid(PCTableAccEntryT{
                        pcHash: e.pcHash, conf: e.conf,
                        usefulCount: newUf, uselessCount: baseUs}));
                    $display("%t AlexLog: CDP Kill acc useful bump, idx: %d pcHash: %h uf: %d -> %d (halved: %b)",
                        cur_cycle, pctIdx, pcHash, e.usefulCount, newUf, sat);
                end
            end
            tagged AccUseless .pcHash: begin
                if (rdResp matches tagged Valid .e &&& e.pcHash == pcHash) begin
                    ...
                    // Halving-on-saturation: if the amplified value would saturate,
                    // halve both counters first (same rationale as AccUseful above).
                    Bit#(7) amplifiedRaw = zeroExtend(e.uselessCount) + 3;
                    Bool sat = (amplifiedRaw >= 63);
                    Bit#(6) baseUf = sat ? (e.usefulCount >> 1) : e.usefulCount;
                    Bit#(6) baseUs = sat ? (e.uselessCount >> 1) : e.uselessCount;
                    Bit#(7) amplifiedAdj = zeroExtend(baseUs) + 3;
                    Bit#(6) newUs = (amplifiedAdj >= 63) ? 63 : truncate(amplifiedAdj);
                    pcTable.wrReq(pctIdx, Valid(PCTableAccEntryT{
                        pcHash: e.pcHash, conf: e.conf,
                        usefulCount: baseUf, uselessCount: newUs}));
                    $display("%t AlexLog: CDP Kill acc useless bump, idx: %d pcHash: %h us: %d -> %d (halved: %b)",
                        cur_cycle, pctIdx, pcHash, e.uselessCount, newUs, sat);
                end
            end
            tagged Decay: begin
                if (rdResp matches tagged Valid .entry) begin
                    function Bit#(3) satDec(Bit#(3) x) = (x == 0) ? 0 : x - 1;
                    Bit#(6) decayUf = (entry.usefulCount > 0) ? entry.usefulCount - 1 : 0;
                    Bit#(6) decayUs = (entry.uselessCount > 0) ? entry.uselessCount - 1 : 0;
                    pcTable.wrReq(pctIdx, Valid(PCTableAccEntryT{
    ...
    rule processTlbReq;
        ...
        pendTlb[id] <= PendingTlbReqT{
            candVaddr:       candVaddr,
            isNeighbourLine: isNeighbourLine,
            crossPage:       crossPage,
            pcHash:          pcH,
            punishable:      punish,
            routeLLC:        rlLLC };
    ...
    rule processTlbResp;
        ...
        PendingTlbReqT p = pendTlb[id];
        tlbReqFreeQ.enq(id);
        if (!resp.haveException) begin
            NextCandAdaptT cand = NextCandAdaptT{
                paddr: resp.paddr, vaddr: p.candVaddr,
                isNeighbourLine: p.isNeighbourLine, routeLLC: p.routeLLC };
            ...
            $display("%0d AlexLog: CDP Rel TLB resp: vaddr %h -> paddr %h lineAddr %h crossPage %b",
                cur_cycle, p.candVaddr, resp.paddr, lineAddr, p.crossPage);
            prefetchFilter.rdReq(filterIdx);
            filterPendingQ.enq(tuple3(filterIdx, cand, p.pcHash));
        end else begin
            $display("%t AlexLog: CDP Rel TLB resp: exception for vaddr %h, dropping prefetch",
                cur_cycle, p.candVaddr);
        end
    endrule
