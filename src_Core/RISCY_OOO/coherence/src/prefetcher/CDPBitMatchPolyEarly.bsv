...
typedef struct { LineAddr lineAddr; } BMPolyEarlyFilterEntryT deriving (Bits, FShow, Eq);

typedef struct {
    Addr paddr;
    Addr vaddr;
    Bool routeLLC;
} NextCandPolyEarlyT deriving (Bits, FShow, Eq);

module mkCDPBitMatchPolyEarly#(
...
provisos (
    ...
    Fifo#(16, NextCandPolyEarlyT) nextCandidateBuffer <- mkOverflowBypassFifo;
    ...
    RWBramCore#(Bit#(10), Maybe#(BMPolyEarlyFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        for (Integer i = 0; i < 8; i = i + 1) begin
            ...
            if (candUpper == missUpper) begin
                ...
                $display("%t AlexLog: CDP BitMatchPolyEarly candidate offset: %d candVaddr: %h matchCount: %d routeLLC: %b",
                    cur_cycle, i, x.line[i], matchCount, routeLLC);
                enqIdx = enqIdx + 1;
            end
        end
        // STRIDE augmentation: unconditional line+1 prefetch, routed to L1
        // (stride-ahead almost always useful near the current access; LLC
        // routing here would lose the latency benefit).
        if (enqIdx < 8) begin
            Addr nextLineVaddr = (getReqAddr(x.req) + 64) & 'hFFFF_FFFF_FFFF_FFC0;
            candFIFO.enqS[enqIdx].enq(tuple2(nextLineVaddr, False));
            $display("%t AlexLog: CDP BitMatchPolyEarly stride candidate candVaddr: %h",
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BitMatchPolyEarly dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BitMatchPolyEarly filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMPolyEarlyFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandPolyEarlyT{paddr: paddr, vaddr: candVaddr, routeLLC: routeLLC});
            $display("%t AlexLog: CDP BitMatchPolyEarly filter MISS, issuing prefetch lineAddr %h routeLLC: %b", cur_cycle, lineAddr, routeLLC);
        end
    endrule
    ...
    method Action reportAccess(Addr addr, Bit#(16) pcHash, HitOrMiss hitMiss, Line line, Vpn reqVpn, MemOp op, Bool isPrefetch);
        ...
        if (filterInited && tlbReqFreeQInited && hitMiss == HIT && op == Ld && !isPrefetch) begin
            ...
            $display("%t AlexLog: CDP BitMatchPolyEarly hit-stride candVaddr: %h (from addr %h)",
