// CDPKillSwitchH10.bsv — H7 + per-prefetch lead-time instrumentation.
//
// Identical algorithm to H7. Adds a single new field issueCycle to the
// filter entry, populated on filter-MISS issue. On useful hit, computes
// leadTime = cur_cycle - issueCycle and adds it to the existing
// "CDP Kill useful hit" log line.
//
// Goal: classify each useful prefetch by approximate fill latency:
//   - leadTime >= 80 cyc → prefetch fill missed in LLC, fetched from DRAM
//     → demand saved ~100 cyc (DRAM-bypass)
//   - leadTime <  50 cyc → prefetch fill hit in LLC
//     → demand saved ~10 cyc (LLC-shuffle)
// Then aggregate per-bench and per-PC to validate observation that
// patricia is the only Olden bench getting DRAM-bypass benefit.
//
...
// H10 ADDITION: issueCycle tracks the cycle at which this prefetch was
// committed to L1 (filter MISS path). On useful hit, leadTime =
// cur_cycle - issueCycle is emitted as a log field. High lead time
// (≥80 cyc) means the prefetch fill went to DRAM and saved DRAM latency;
// low lead time (≤50 cyc) means it hit LLC and only saved LLC latency.
typedef struct {
    LineAddr lineAddr;
    Bit#(16) pcHash;
    Bool     punishable;            // False for neighbour-chain entries (no clean attribution)
    Bit#(32) issueCycle;            // H10: cycle of issue, for lead-time computation
...
module mkCDPStatefulRelativeKillSwitchH10#(
...
provisos (
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            ...
            // H10: record issueCycle for lead-time computation on useful hit.
            let curCyc <- cur_cycle;
    ...
    rule finishUsefulLookup(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr &&& fe.punishable) begin
            ...
            // H10: leadTime = cycles between issue and useful consumption.
            // High lead time indicates the prefetch fill went all the way to
            // DRAM (saving ~100 cyc on demand); low lead time means it hit
            // LLC (saving only ~10 cyc).
            let curCyc <- cur_cycle;
            Bit#(32) leadTime = curCyc - fe.issueCycle;
            $display("%t AlexLog: CDP Kill useful hit: lineAddr %h pcHash %h leadTime %d",
                cur_cycle, lineAddr, fe.pcHash, leadTime);
        end
