// CDPBMAlignSuppConfT1.bsv -- BMAlignSupp + per-PC attribution gating.
...
typedef struct {
    LineAddr lineAddr;
    Bit#(16) pcHash;
} BMAlignSuppConfT1FilterEntryT deriving (Bits, FShow, Eq);
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(BMAlignSuppConfT1FilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
        ...
        if (!suppress) begin
            for (Integer i = 0; i < 8; i = i + 1) begin
                ...
                if (candUpper == missUpper && isPtrShape(x.line[i])) begin
                    ...
                    $display("%t AlexLog: CDP BMAlignSuppConfT1 candidate offset: %d candVaddr: %h matchCount: %d pcHash: %h score: %d L1",
                        cur_cycle, i, x.line[i], matchCount, pc, score);
                    enqIdx = enqIdx + 1;
                end
            end
        end else begin
            $display("%t AlexLog: CDP BMAlignSuppConfT1 SUPPRESS matchCount: %d pcHash: %h score: %d (%s)",
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BMAlignSuppConfT1 dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BMAlignSuppConfT1 filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMAlignSuppConfT1FilterEntryT{ lineAddr: lineAddr, pcHash: pc }));
            nextCandidateBuffer.enq(NextCandAlignSuppConfT{paddr: paddr, vaddr: candVaddr, routeLLC: routeLLC, pcHash: pc});
            $display("%t AlexLog: CDP BMAlignSuppConfT1 filter MISS, issuing prefetch lineAddr %h pcHash: %h", cur_cycle, lineAddr, pc);
        end
    endrule
    ...
    rule processAttribResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            ...
            $display("%t AlexLog: CDP BMAlignSuppConfT1 %s lineAddr %h pcHash %h score %d -> %d",
