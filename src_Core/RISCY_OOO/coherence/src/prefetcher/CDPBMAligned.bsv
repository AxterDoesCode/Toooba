...
typedef struct { LineAddr lineAddr; } BMAlignedFilterEntryT deriving (Bits, FShow, Eq);

module mkCDPBMAligned#(
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(BMAlignedFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        for (Integer i = 0; i < 8; i = i + 1) begin
            ...
            // BMAligned: additionally require the candidate to be at least
            // 16-byte aligned (low 4 bits zero). Heap objects from glibc malloc
            // are 16-byte aligned by default; C++ objects usually 16-byte aligned;
            // most dynamic allocators give at least 16-byte alignment. Random
            // integer data passes 8-byte alignment much more often (any even-
            // multiple-of-8 int) but 16-byte alignment much less. This filters
            // data-looking-like-pointers without needing any learning table.
            // (64-byte alignment was too strict — treeadd's pointers aren't all
            //  cache-line-aligned.)
            Bool alignedCand = (x.line[i][3:0] == 0);
            if (candUpper == missUpper && alignedCand) begin
                candFIFO.enqS[enqIdx].enq(x.line[i]);
                $display("%t AlexLog: CDP BMAligned candidate offset: %d candVaddr: %h crossPage: %b",
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BMAligned dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BMAligned filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMAlignedFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandT{paddr: paddr, vaddr: candVaddr, isNeighbourLine: False});
            $display("%t AlexLog: CDP BMAligned filter MISS, issuing prefetch lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
