// CDPKillSwitchH15.bsv — Variant H15: H7 with confidence counters narrowed
// from 3-bit to 2-bit. First step in BRAM-table-entry size reduction sweep.
//
// Bit accounting per PC entry:
//   H7:  pcHash(16) + 15 × conf(3) + uf(4) + us(4) = 69 bits/entry
//   H15: pcHash(16) + 15 × conf(2) + uf(4) + us(4) = 54 bits/entry
//   Saving: 15 bits/entry × 1024 entries = 15,360 bits ≈ 1.92 KB
//
// Behavioural side effect: confidence counter saturates at 3 instead of 7.
// H7 default is confidenceThreshold=1 (issue at any nonzero conf), which is
// fully expressible in 2 bits — H15 keeps threshold=1. The only behavioural
// loss is finer-grained confidence ranking when picking among multiple
// matching offsets (saturation reached after 3 trainings instead of 7).
//
...
module mkCDPStatefulRelativeKillSwitchH15#(
