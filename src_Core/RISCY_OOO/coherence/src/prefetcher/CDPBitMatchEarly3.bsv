...
typedef struct { LineAddr lineAddr; } BMEarly3FilterEntryT deriving (Bits, FShow, Eq);

module mkCDPBitMatchEarly3#(
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(BMEarly3FilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        for (Integer i = 0; i < 8; i = i + 1) begin
            ...
            if (candUpper == missUpper) begin
                ...
                $display("%t AlexLog: CDP BitMatchEarly3 candidate offset: %d candVaddr: %h crossPage: %b",
        ...
        if (enqIdx < 8) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly3 stride+1 candVaddr: %h",
                cur_cycle, nextLine1);
            enqIdx = enqIdx + 1;
        end
        if (enqIdx < 8) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly3 stride+2 candVaddr: %h",
                cur_cycle, nextLine2);
        end
    endrule
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly3 dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BitMatchEarly3 filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMEarly3FilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandT{paddr: paddr, vaddr: candVaddr, isNeighbourLine: False});
            $display("%t AlexLog: CDP BitMatchEarly3 filter MISS, issuing prefetch lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
    ...
    method Action reportAccess(Addr addr, Bit#(16) pcHash, HitOrMiss hitMiss, Line line, Vpn reqVpn, MemOp op, Bool isPrefetch);
        ...
        if (filterInited && tlbReqFreeQInited && hitMiss == HIT && op == Ld && !isPrefetch) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly3 hit-stride line+1: %h line+2: %h (from addr %h)",
