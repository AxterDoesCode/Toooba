// CDPPathHistXor.bsv — Variant D2: order-independent 4-PC XOR path signature.
//
// Sibling of Variant D (`CDPPathHist.bsv`) but replaces the shift-XOR rolling
// hash with a plain XOR over a 4-entry ring buffer of the last demand-load
// pcHashes. Motivation from the user: a shift-XOR encodes *order*; many
// LDS access patterns really are "the same four PCs have been recently
// active in some order" without strict ordering — the XOR collapses all
// orderings to one signature, so the same context identifies the same
// prefetch future even when interleaving varies.
//
// Same core mechanism as mkCDPStatefulRelative but the PC-table key is a
// *path signature*: the XOR of the last 4 demand-load pcHashes. The ring
// buffer is shifted on every demand access (miss or hit), but the signature
// itself is the bare XOR of those 4 entries — order-INDEPENDENT. This means
// the same set of recent PCs produces the same signature regardless of
// the exact arrival order: if the same four PCs recur in any permutation,
// the prefetcher recognises the context.
//
//   pathHist[0..3]: ring buffer of last 4 pcHashes
//   on update:  pathHist <= cons(current_pcHash, init(pathHist))  // shift
//   pathSig   = pathHist[0] ^ pathHist[1] ^ pathHist[2] ^ pathHist[3]
//
...
module mkCDPStatefulRelativePathHistXor#(
...
provisos (
    ...
    // Ring buffer of the last 4 demand-load pcHashes. Signature is their
    // bare XOR (order-independent). Any permutation of the same 4 PCs
    // produces the same pathSig, which is the intent: "this context set
    // recently touched memory".
    Reg#(Vector#(4, Bit#(16))) pathHist <- mkReg(replicate(0));
    ...
    // Order-independent 4-entry XOR signature of the recent demand-load PCs.
    ...
    rule deqCacheLines;
        ...
        Bit#(16) scanSig = getPcHash(x.req) ^ pathSigFn(pathHist);
        if (getReqOp(x.req) == Ld) begin
            ...
            for (Integer i = 0; i < 8; i = i + 1) begin
                ...
                if (candUpper == missUpper) begin
                    ...
                    $display("%t AlexLog: CDP PathHistXor candidate vaddr relOffset: %d scanSig: %h candVaddr: %h",
    ...
    rule ttAccess(inited);
        ...
        if (respQ.isTrainingLookup) begin
            if (rdResp matches tagged Valid {.hitWay, .ttRdResp}) begin
                ...
                $display("%t AlexLog: CDP PathHistXor Training hit: missVaddr %h seen before by sig %h at relOffset %d",
            ...
            Bit#(16) scanSig = getPcHash(respQ.req) ^ pathSigFn(pathHist);
    ...
    rule pcTableResp(inited);
        ...
        case (tag) matches
            tagged Training {.pcHash, .relOffset}: begin
                ...
                $display("%t AlexLog: CDP PathHistXor PC table updated, idx: %d sig: %h relOffset: %d conf: %d -> %d",
            ...
            tagged PrefetchIssue {.addr, .line, .reqVpn}: begin
                if (rdResp matches tagged Valid .entry) begin
                    ...
                    if (!foundHighConf) begin
                        ...
                        $display("%t AlexLog: CDP PathHistXor no high-conf offset: sig %h maxConf %d",
                            cur_cycle, entry.pcHash, maxConf);
                    end
                    if (foundHighConf) begin
                        Bit#(4) bestIdx = pack(bestRelOffset + 7);
                        Bool isNeighbour = !(bestAbsTarget >= 0 &&& bestAbsTarget <= 7);
                        $display("%t AlexLog: CDP PathHistXor prefetch decision: sig %h relOffset %d conf %d isNeighbour %b",
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .storedAddr &&& storedAddr == lineAddr) begin
            $display("%t AlexLog: CDP PathHistXor filter HIT: dropped duplicate prefetch for lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(lineAddr));
            nextCandidateBuffer.enq(cand);
            $display("%t AlexLog: CDP PathHistXor filter MISS: issuing prefetch for lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
    ...
    method Action reportIncomingCacheLine(reqT req, Line line, Bool cRqIsPrefetch, Bool wasMiss, Bool wasNeighbourPrefetch);
        if (inited && getReqOp(req) == Ld && !cRqIsPrefetch && wasMiss && !wasNeighbourPrefetch) begin
            ...
            pcTableIdxT pctIdx = hash(getPcHash(req) ^ pathSigFn(pathHist));
    ...
    method ActionValue#(PendingPrefetch) getNextPrefetchAddr;
        ...
        $display("%t AlexLog: CDP PathHistXor Prefetch addr issued. lineAddr: %h", cur_cycle, getLineAddr(x.paddr));
