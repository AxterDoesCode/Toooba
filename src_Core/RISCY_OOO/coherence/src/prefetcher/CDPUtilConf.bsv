...
} PCTableUtilConfEntryT deriving (Bits, FShow, Eq);
...
typedef struct {
    ...
    Bool routeLLC;
} NextCandUtilConfT deriving (Bits, FShow, Eq);

// Dedup filter entry: minimal.
typedef struct { LineAddr lineAddr; } UtilConfFilterEntryT deriving (Bits, FShow, Eq);
...
typedef struct {
    ...
    Bool         punishable;
} UtilConfAttribEntryT deriving (Bits, FShow, Eq);
...
} PCTableRdUtilConfTagT deriving (Bits, FShow);

module mkCDPStatefulRelativeUtilConf#(
...
provisos (
    ...
    RWBramCore#(pcTableIdxT, Maybe#(PCTableUtilConfEntryT)) pcTable <- mkRWBramCoreForwarded();
    // Dedup filter -- 1024 entries (Bit#(10)) as before.
    RWBramCore#(Bit#(10), Maybe#(UtilConfFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    // Attribution table -- 4096 entries (Bit#(12)), independent of the filter.
    RWBramCore#(Bit#(12), Maybe#(UtilConfAttribEntryT)) attribTable <- mkRWBramCoreForwarded();

    Fifo#(16, NextCandUtilConfT) nextCandidateBuffer <- mkOverflowBypassFifo;
    FIFO#(Tuple2#(pcTableIdxT, PCTableRdUtilConfTagT)) pcTableRdReqFIFO <- mkSizedFIFO(64);
    FIFO#(Tuple2#(pcTableIdxT, PCTableRdUtilConfTagT)) pcTableRdTagQ    <- mkFIFO;
    FIFO#(Tuple4#(Bit#(10), NextCandUtilConfT, Bit#(16), RelLineOffset)) filterPendingQ <- mkFIFO;
    ...
    rule pcTableResp(inited);
        ...
        case (tag) matches
            tagged Training {.pcHash, .relOffset}: begin
                ...
                pcTable.wrReq(pctIdx, Valid(PCTableUtilConfEntryT{
                    pcHash: pcHash,
                    conf: update(curConf, vecIdx, newVal)}));
                $display("%t AlexLog: CDP UtilConf PC table trained (eligible), idx: %d pcHash: %h relOffset: %d conf: %d -> %d",
            ...
            tagged UtilBump {.pcHash, .relOffset, .isDecrement}: begin
                ...
                if (rdResp matches tagged Valid .e &&& e.pcHash == pcHash) begin
                    ...
                    pcTable.wrReq(pctIdx, Valid(PCTableUtilConfEntryT{
                        pcHash: e.pcHash,
                        conf: update(curConf, vecIdx, newVal)}));
                    $display("%t AlexLog: CDP UtilConf %s bump, idx: %d pcHash: %h relOffset: %d conf: %d -> %d",
            ...
            tagged PrefetchIssue {.addr, .line, .reqVpn}: begin
                if (rdResp matches tagged Valid .entry) begin
                    ...
                    if (foundHighConf) begin
                        $display("%t AlexLog: CDP UtilConf prefetch decision: pcHash %h relOffset %d ISSUE",
    ...
    rule processTlbResp;
        ...
        if (!resp.haveException) begin
            NextCandUtilConfT cand = NextCandUtilConfT{
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP UtilConf filter HIT: dropped duplicate prefetch for lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(UtilConfFilterEntryT{ lineAddr: lineAddr }));
            Bit#(12) attribIdx = hash(lineAddr);
            attribTable.wrReq(attribIdx, Valid(UtilConfAttribEntryT{
                lineAddr: lineAddr, pcHash: pcH, relOffset: relOff, punishable: True }));
            nextCandidateBuffer.enq(cand);
            $display("%t AlexLog: CDP UtilConf filter MISS: issuing prefetch for lineAddr %h pcHash %h relOff %d",
    ...
    rule finishEviction(attribInited);
        ...
        if (rdResp matches tagged Valid .ae &&& ae.lineAddr == lineAddr) begin
            if (ae.punishable) begin
                ...
                $display("%t AlexLog: CDP UtilConf useless attrib: lineAddr %h pcHash %h relOff %d",
    ...
    rule finishUsefulLookup(attribInited);
        ...
        if (rdResp matches tagged Valid .ae &&& ae.lineAddr == lineAddr &&& ae.punishable) begin
            ...
            $display("%t AlexLog: CDP UtilConf useful attrib: lineAddr %h pcHash %h relOff %d",
                cur_cycle, lineAddr, ae.pcHash, ae.relOffset);
        end
    endrule
    ...
    method ActionValue#(PendingPrefetch) getNextPrefetchAddr;
        ...
        $display("%t AlexLog: CDP UtilConf prefetch issued. lineAddr: %h",
