// CDPKillSwitchH17.bsv — Variant H17: H16 + hashed-tag tables.
//
// H16's BRAM is dominated by full-precision addresses stored as set-assoc
// tags: the prefetchFilter holds a 58-bit lineAddr per entry (9.73 KB across
// 1024 entries), and the training table holds a 64-bit storedVaddr per
// entry (1.36 KB across 128). H17 replaces both with 16-bit hash tags.
// The address itself is no longer stored anywhere in the entry.
//
// Tag derivation: a small XOR-fold over different bit slices than the
// index hash, so a (idx, tag) collision requires the addresses to agree
// on both functions. With 16-bit tags the per-slot false-match rate is
// ~1/65536; aggregate collision rate per query is ~1.6%.
//
// Failure modes from collision:
//   * Filter false positive: two distinct lines hash to the same idx+tag.
//     A new prefetch for line A is suppressed because the filter holds
//     entry from line B with a matching tag. Loses some prefetch
//     opportunity; does NOT introduce pollution.
//   * TT false match: two distinct vaddrs collide. PC table gets
//     incremented on wrong evidence. Subtly degrades PC table quality.
//
// BRAM accounting at the configuration used throughout the dissertation:
//   trainingTable: 128 × 85 b → 128 × 37 b (–0.75 KB)
//   PC table:      unchanged (already uses pcHash as tag, 7.04 KB)
//   prefetchFilter: 1024 × 76 b → 1024 × 34 b (–5.25 KB)
// Total: 18.13 KB → 11.88 KB (–34%).
//
...
// Local TT entry that mirrors CDP.bsv::TrainingTableEntryT but stores a
// 16-bit hash tag instead of the full 64-bit storedVaddr.
typedef struct {
    Bool          valid;
    Bit#(16)      addrTag;          // tagOfVaddr(storedVaddr)
    Bit#(16)      pcHash;
    RelLineOffset lineOffset;
} TrainingTableHashedEntryT deriving (Bits, FShow, Eq);

// Tag from a full 64-bit virtual address. XOR-fold over high bit slices so
// the result is approximately independent from hash() (which selects the
// low 6 bits of vaddr39 for the TT index).
function Bit#(16) tagOfVaddr(Addr a);
    Bit#(16) s0 = a[31:16];
    Bit#(16) s1 = a[47:32];
    Bit#(16) s2 = a[63:48];
    return s0 ^ s1 ^ s2;
...
module mkCDPStatefulRelativeKillSwitchH17#(
...
provisos (
    ...
    function Bool ttIsMatch(TrainingTableHashedEntryT e, Bit#(16) tag) = e.valid && e.addrTag == tag;
    function Bool ttIsReplaceCandidate(TrainingTableHashedEntryT e) = !e.valid;
    RWSetAssocBramCore#(trainingTableIdxT, Bit#(1), TrainingTableHashedEntryT, Bit#(16)) trainingTable
    ...
    rule processTtRdReq(inited);
        ...
        trainingTable.rdReq(idx, tagOfVaddr(vaddr));
    endrule
    ...
    rule ttAccess(inited);
        ...
        if (respQ.isTrainingLookup) begin
            ...
            TrainingTableHashedEntryT newEntry = TrainingTableHashedEntryT{
                valid: True, addrTag: tagOfVaddr(respQ.candVaddr),
