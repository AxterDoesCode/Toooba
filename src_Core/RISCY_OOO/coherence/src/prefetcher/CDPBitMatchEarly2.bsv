...
typedef struct { LineAddr lineAddr; } BMEarly2FilterEntryT deriving (Bits, FShow, Eq);

module mkCDPBitMatchEarly2#(
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(BMEarly2FilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    rule deqLineL1(filterInited && tlbReqFreeQInited);
        ...
        for (Integer i = 0; i < 8; i = i + 1) begin
            ...
            if (candUpper == missUpper) begin
                ...
                $display("%t AlexLog: CDP BitMatchEarly2 candidate offset: %d candVaddr: %h crossPage: %b",
                    cur_cycle, i, x.line[i], getVpn(x.line[i]) != getReqVpn(x.req));
                enqIdx = enqIdx + 1;
            end
        end
        // Stride degree 2: emit BOTH line+1 and line+2 on miss.
        Addr baseLine = getReqAddr(x.req) & 'hFFFF_FFFF_FFFF_FFC0;
        if (enqIdx < 8) begin
            Addr nextLine1 = baseLine + 64;
            candFIFO.enqS[enqIdx].enq(nextLine1);
            $display("%t AlexLog: CDP BitMatchEarly2 stride+1 candVaddr: %h",
                cur_cycle, nextLine1);
            enqIdx = enqIdx + 1;
        end
        if (enqIdx < 8) begin
            Addr nextLine2 = baseLine + 128;
            candFIFO.enqS[enqIdx].enq(nextLine2);
            $display("%t AlexLog: CDP BitMatchEarly2 stride+2 candVaddr: %h",
                cur_cycle, nextLine2);
        end
    endrule
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly2 dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BitMatchEarly2 filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMEarly2FilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandT{paddr: paddr, vaddr: candVaddr, isNeighbourLine: False});
            $display("%t AlexLog: CDP BitMatchEarly2 filter MISS, issuing prefetch lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
    ...
    method Action reportAccess(Addr addr, Bit#(16) pcHash, HitOrMiss hitMiss, Line line, Vpn reqVpn, MemOp op, Bool isPrefetch);
        ...
        if (filterInited && tlbReqFreeQInited && hitMiss == HIT && op == Ld && !isPrefetch) begin
            ...
            $display("%t AlexLog: CDP BitMatchEarly2 hit-stride candVaddr: %h (from addr %h)",
