// CDPIRatio.bsv -- Variant IRATIO: per-PC issue-ratio kill-switch.
//
// Motivation: per-PC analysis of Variant M logs revealed that treeadd's
// catastrophic -22% slowdown is driven by PC 9094 (4326 issued / 186 useful
// / 21 useless attributed = 4.3% acc). The H-family kill-switch uses useless
// attribution as its kill signal, but the attribution channel is severely
// undersampled — only 21 useless bumps for ~4000 actual pollution events on
// this PC (0.5% capture rate), because the 1024-entry prefetch filter is
// hash-evicted by newer prefetches before the corresponding cache eviction
// fires. This is why H caps at +4% geomean.
//
// IRATIO bypasses the useless-attribution channel entirely. It uses:
//   - issuedCount (per PC, dense, 100% coverage — bumps on every issue)
//   - usefulCount (per PC, dense — bumps on every reportUsefulPrefetch)
// and the RATIO of useful/issued as the kill trigger. For treeadd PC 9094
// that would be 186/4326 = 4.3% — correctly identified as pollution. For
// patricia PC 84e0 (1304/1341 = 97.2%) — correctly preserved. For treeadd
// PC 9238 (1497/1951 = 76.7%) — correctly preserved.
//
// Kill condition: issuedCount >= 64 AND usefulCount * 10 < issuedCount
// (i.e. <10% useful after at least 64 observations). Once killed, the PC's
// prefetches are permanently suppressed (no probabilistic re-check).
//
// Structural debt that still remains: this shares attribTable/conf
// machinery with Variant O but overloads the pcTable pipeline with two
// more request types (IssueBump, UsefulBump). Throughput of the 64-deep
// pcTableRdReqFIFO may be stressed on high-issue benchmarks.
//
// ---------- Inherited from Variant O ----------
...
// PC-table entry: adds per-PC issuedCount + usefulCount + killed flag.
// Raw issued count is DENSE (fires on every issued prefetch — 100% coverage),
// useful count is DENSE from the reportUsefulPrefetch side-channel (100% of
// demand hits on prefetched lines, attributed via the 4096-entry attribTable).
// The ratio useful/issued is therefore a reliable accuracy estimator even
// when the uselessEvict attribution channel is noisy.
// Counters saturate at 255 (8-bit) and are halved when either saturates, to
// preserve the ratio while giving room to adapt to behaviour shifts.
typedef struct {
    Bit#(16)         pcHash;
    ...
    Bit#(8)          usefulCount;
    Bool             killed;
} PCTableIRatioEntryT deriving (Bits, FShow, Eq);
...
typedef struct {
    ...
    Bool routeLLC;
} NextCandIRatioT deriving (Bits, FShow, Eq);

// Dedup filter entry: minimal.
typedef struct { LineAddr lineAddr; } IRatioFilterEntryT deriving (Bits, FShow, Eq);
...
typedef struct {
    ...
    Bool         punishable;
} IRatioAttribEntryT deriving (Bits, FShow, Eq);

// Attribution events route through the pcTable pipeline (read-modify-write)
// so all updates serialise cleanly.
// UtilBump isDecrement=False -> +1 conf AND +1 usefulCount (useful hit);
//         isDecrement=True  -> -1 conf only (useless evict — per-PC usefulCount
//         intentionally not touched on useless).
// IssueBump: pcHash — increments issuedCount + evaluates kill gate.
typedef union tagged {
    ...
    Bit#(16)                                  IssueBump;
} PCTableRdIRatioTagT deriving (Bits, FShow);

module mkCDPStatefulRelativeIRatio#(
...
provisos (
    ...
    RWBramCore#(pcTableIdxT, Maybe#(PCTableIRatioEntryT)) pcTable <- mkRWBramCoreForwarded();
    // Dedup filter -- 1024 entries (Bit#(10)) as before.
    RWBramCore#(Bit#(10), Maybe#(IRatioFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    // Attribution table -- 4096 entries (Bit#(12)), independent of the filter.
    RWBramCore#(Bit#(12), Maybe#(IRatioAttribEntryT)) attribTable <- mkRWBramCoreForwarded();

    Fifo#(16, NextCandIRatioT) nextCandidateBuffer <- mkOverflowBypassFifo;
    FIFO#(Tuple2#(pcTableIdxT, PCTableRdIRatioTagT)) pcTableRdReqFIFO <- mkSizedFIFO(64);
    FIFO#(Tuple2#(pcTableIdxT, PCTableRdIRatioTagT)) pcTableRdTagQ    <- mkFIFO;
    FIFO#(Tuple4#(Bit#(10), NextCandIRatioT, Bit#(16), RelLineOffset)) filterPendingQ <- mkFIFO;
    ...
    rule pcTableResp(inited);
        ...
        case (tag) matches
            tagged Training {.pcHash, .relOffset}: begin
                ...
                pcTable.wrReq(pctIdx, Valid(PCTableIRatioEntryT{
                    pcHash: pcHash,
                    conf: update(curConf, vecIdx, newVal),
                    issuedCount: curIssued,
                    usefulCount: curUseful,
                    killed: curKilled}));
                $display("%t AlexLog: CDP IRatio PC table trained, idx: %d pcHash: %h relOffset: %d conf: %d -> %d",
                         cur_cycle, pctIdx, pcHash, relOffset, curVal, newVal);
            end
            tagged UtilBump {.pcHash, .relOffset, .isDecrement}: begin
                // Merged path: update conf AND (if useful) usefulCount.
                if (rdResp matches tagged Valid .e &&& e.pcHash == pcHash) begin
                    PCRelOffsetConfT curConf = e.conf;
                    Bit#(4) vecIdx = pack(relOffset + 7);
                    Bit#(3) curVal = curConf[vecIdx];
                    Bit#(3) newConfVal = isDecrement
                        ? ((curVal == 0) ? 0 : curVal - 1)
                        : ((curVal == maxBound) ? maxBound : curVal + 1);
                    Bit#(8) newUseful = isDecrement
                        ? e.usefulCount
                        : ((e.usefulCount == maxBound) ? maxBound : e.usefulCount + 1);
                    pcTable.wrReq(pctIdx, Valid(PCTableIRatioEntryT{
                        pcHash: e.pcHash,
                        conf: update(curConf, vecIdx, newConfVal),
                        issuedCount: e.issuedCount,
                        usefulCount: newUseful,
                        killed: e.killed}));
                end
            end
            tagged IssueBump .pcHash: begin
                // Called on every issued prefetch. Saturates the 8-bit counters
                // by halving BOTH when either reaches max — preserves ratio.
                // Triggers kill if issued >= 64 and useful*10 < issued (<10% acc).
                if (rdResp matches tagged Valid .e &&& e.pcHash == pcHash) begin
                    Bit#(8) newIssued = e.issuedCount == maxBound ? 8'd128 : e.issuedCount + 1;
                    Bit#(8) newUseful = e.issuedCount == maxBound ? (e.usefulCount >> 1) : e.usefulCount;
                    // Kill-switch evaluation: >=128 issues observed AND useful ratio < 5%.
                    // Rationale: attribution has ~500-1000 cycle lag; at 128 issues
                    // (~6000 cycles elapsed for treeadd-like rates), attribution has
                    // had time to report on the first ~80-100 issues. Choosing 5%
                    // (not 10%) keeps borderline PCs like treeadd's 9238 (76.7%)
                    // safe from lag-induced miscounts (9238 was falsely killed at
                    // 10% acc under the older 64-issue/10% rule).
                    Bit#(16) usefulTimes20 = zeroExtend(newUseful) * 20;
                    Bit#(16) issued16 = zeroExtend(newIssued);
                    Bool shouldKill = (newIssued >= 128) && (usefulTimes20 < issued16);
                    Bool killed = e.killed || shouldKill;
                    pcTable.wrReq(pctIdx, Valid(PCTableIRatioEntryT{
                        pcHash: e.pcHash,
                        conf: e.conf,
                        ...
                        usefulCount: newUseful,
                        killed: killed}));
                    if (shouldKill && !e.killed) begin
                        $display("%t AlexLog: CDP IRatio KILL pcHash %h issued %d useful %d",
                                 cur_cycle, pcHash, newIssued, newUseful);
                    end
                end
            end
            tagged PrefetchIssue {.addr, .line, .reqVpn}: begin
                if (rdResp matches tagged Valid .entry &&& !entry.killed) begin
                    ...
                    if (foundHighConf) begin
                        $display("%t AlexLog: CDP IRatio prefetch decision: pcHash %h relOffset %d ISSUE",
                            cur_cycle, entry.pcHash, bestRelOffset);
                        // Count the issue in the per-PC counter (via pcTable pipeline).
                        pcTableRdReqFIFO.enq(tuple2(pctIdx, tagged IssueBump entry.pcHash));
                        if (bestAbsTarget >= 0 &&& bestAbsTarget <= 7) begin
                            ...
                            if (candUpper == addrUpper)
                                tlbReqFIFO.enq(tuple7(candidate, False, getVpn(candidate) != reqVpn,
                                                       entry.pcHash, True, False, bestRelOffset));
                        end else begin
    ...
    rule processTlbResp;
        ...
        if (!resp.haveException) begin
            NextCandIRatioT cand = NextCandIRatioT{
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP IRatio filter HIT: dropped duplicate prefetch for lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(IRatioFilterEntryT{ lineAddr: lineAddr }));
            Bit#(12) attribIdx = hash(lineAddr);
            attribTable.wrReq(attribIdx, Valid(IRatioAttribEntryT{
                lineAddr: lineAddr, pcHash: pcH, relOffset: relOff, punishable: True }));
            nextCandidateBuffer.enq(cand);
            $display("%t AlexLog: CDP IRatio filter MISS: issuing prefetch for lineAddr %h pcHash %h relOff %d",
    ...
    rule finishEviction(attribInited);
        ...
        if (rdResp matches tagged Valid .ae &&& ae.lineAddr == lineAddr) begin
            if (ae.punishable) begin
                ...
                $display("%t AlexLog: CDP IRatio useless attrib: lineAddr %h pcHash %h relOff %d",
    ...
    rule finishUsefulLookup(attribInited);
        ...
        if (rdResp matches tagged Valid .ae &&& ae.lineAddr == lineAddr &&& ae.punishable) begin
            pcTableIdxT pctIdx = hash(ae.pcHash);
            // UtilBump with isDecrement=False now also bumps usefulCount internally.
            pcTableRdReqFIFO.enq(tuple2(pctIdx, tagged UtilBump tuple3(ae.pcHash, ae.relOffset, False)));
            $display("%t AlexLog: CDP IRatio useful attrib: lineAddr %h pcHash %h relOff %d",
                cur_cycle, lineAddr, ae.pcHash, ae.relOffset);
        end
    endrule
    ...
    method ActionValue#(PendingPrefetch) getNextPrefetchAddr;
        ...
        $display("%t AlexLog: CDP IRatio prefetch issued. lineAddr: %h",
