// CDPHybrid.bsv -- Variant L: classifier-dispatched CDP.
//
// Motivation: every kill-switch variant (H4/J/J2) plateaus at +7.3-7.8% geomean
// vs NoPrefetch -- they can suppress purely-bad PCs but not MIXED PCs (PCs
// that generate some useful and some useless prefetches). The research
// evidence (IPCP ISCA'20, Ebrahimi HPCA'09) says the right lever is per-PC
// CLASS, not per-PC kill-switch: identify stride-class PCs and stop CDP from
// content-scanning on them at all.
//
// This variant adds a small 64-entry per-PC stride classifier. Per-PC state:
// lastLineAddr, stride, matchCount. On demand Ld (hit or miss), update the
// state. On prefetch decision time, if the classifier has seen >= 3
// consecutive stride matches for this PC AND the stride is non-zero, the PC
// is STRIDE-class -> CDP's content-scan prefetch is SUPPRESSED (we emit
// nothing; a future L2 could emit a stride prefetch here, but L1 tests the
// suppression hypothesis alone).
//
// Size: 64 entries * (16 pcHash + 34 lastLineAddr + 13 stride + 3 matchCount +
// 1 valid) = ~67 bits * 64 = 4.3 Kbit = 540 bytes. Register-file (not BRAM)
// so reads/writes are combinational -- no pipeline latency in classifier.
...
} StrideClassEntry deriving (Bits, Eq, FShow);

module mkCDPStatefulRelativeHybrid#(
...
provisos (
    ...
    Vector#(64, Reg#(StrideClassEntry)) strideClassifier <- replicateM(mkReg(unpack(0)));
    ...
    rule strideUpdate(inited);
        ...
        if (!cur.valid || cur.pcHash != pcHash) begin
            ...
            if (observedStride == cur.stride && cur.stride != 0) begin
                ...
                $display("%t AlexLog: CDP Hybrid stride match pcHash %h stride %d matchCount %d -> %d",
            ...
            tagged PrefetchIssue {.addr, .line, .reqVpn}: begin
                if (rdResp matches tagged Valid .entry) begin
                    ...
                    if (foundHighConf) begin
                        // ** Classifier gate: stride-class PCs get no CDP prefetch. **
                        Bool isStrided = isStrideClass(entry.pcHash);
                        Bit#(4) bestIdx = pack(bestRelOffset + 7);
                        $display("%t AlexLog: CDP Hybrid prefetch decision: pcHash %h relOffset %d conf %d %s",
                            cur_cycle, entry.pcHash, bestRelOffset, entry.conf[bestIdx],
                            isStrided ? "SUPPRESSED_STRIDE" : "ISSUE");
                        if (!isStrided) begin
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .storedAddr &&& storedAddr == lineAddr) begin
            $display("%t AlexLog: CDP Hybrid filter HIT: dropped duplicate prefetch for lineAddr %h", cur_cycle, lineAddr);
        end else begin
