// CDPKillSwitchH13.bsv — H7 + CDP fires on store misses too.
//
// Identical to H7 in every other respect; the only change is in the
// reportIncomingCacheLine method body and deqCacheLines rule, where the
// `op == Ld` gate is widened to include `op == St`.
//
// Motivation: bh / perimeter / tsp are 95-99.8% St-miss-dominated, so
// CDP never trains on them under H7. This variant tests whether
// extending CDP's training input to store misses captures any of those
// benches' demand-Ld-miss patterns. Expected payoff is small (≤0.1 pp
// geomean) because (a) bh's init stores hit fresh memory with no
// pointer values, and (b) perimeter/tsp's demand-Ld-miss target sets
// are tiny (14 / 26 unique cache lines), so most discovered candidates
// will never become a future demand load. This experiment confirms the
...
module mkCDPStatefulRelativeKillSwitchH13#(
...
provisos (
    ...
    rule deqCacheLines;
        ...
        // H13: train on St misses too, not just Ld.
        if (getReqOp(x.req) == Ld || getReqOp(x.req) == St) begin
    ...
    method Action reportIncomingCacheLine(reqT req, Line line, Bool cRqIsPrefetch, Bool wasMiss, Bool wasNeighbourPrefetch);
        ...
        // H13: accept St misses as well as Ld misses for training.
        if (inited && (getReqOp(req) == Ld || getReqOp(req) == St) && !cRqIsPrefetch && wasMiss && !wasNeighbourPrefetch) begin
