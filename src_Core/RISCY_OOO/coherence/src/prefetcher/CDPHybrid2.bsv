// CDPHybrid2.bsv -- Variant L2: classifier-dispatched CDP with stride EMIT.
//
// Extends L (CDPHybrid.bsv) which only SUPPRESSED CDP for STRIDE-class PCs.
// L2 additionally EMITS a stride-based prefetch for those PCs: target =
// currentLineAddr + observedStride * LINE_SZ. One line ahead only (simplest).
//
// Parser analysis of L showed: the classifier fires correctly (0 on
// pure pointer-chase benchmarks, 1755 on treeadd). But suppression alone
// leaves the stride-prefetch benefit on the table. L2 tests whether emitting
// that stride prefetch recovers cycles on treeadd (most strided bench) while
// keeping the benefits on pointer-chasers.
...
typedef struct {
    Bool     valid;
    Bit#(16) pcHash;
    Bit#(34) lastLineAddr;   // truncated — collisions are OK, only matters for stride match
    Int#(13) stride;         // signed delta between consecutive line addrs
    Bit#(3)  matchCount;     // saturating 0..7
} StrideClassEntry2 deriving (Bits, Eq, FShow);

module mkCDPStatefulRelativeHybrid2#(
    ...
    Parameter#(strideClassThreshold) ______
...
provisos (
    ...
    // Per-PC stride classifier -- register-file so it can be read/updated
    // combinationally with zero pipeline cost.
    Vector#(64, Reg#(StrideClassEntry2)) strideClassifier <- replicateM(mkReg(unpack(0)));
    // Update queue: reportIncomingCacheLine enqueues (pcHash, lineAddr) on every
    // demand Ld; strideUpdate rule drains it and maintains the classifier.
    FIFO#(Tuple2#(Bit#(16), LineAddr)) strideUpdateQ <- mkSizedFIFO(16);
    ...
    FIFO#(Tuple3#(Addr, Bool, Bool)) tlbReqFIFO <- mkSizedFIFO(32);
    ...
    // Classifier helpers
    function Bit#(6) strideIdx(Bit#(16) pcHash);
        Bit#(16) h = hash(pcHash);
        return truncate(h);
    endfunction

    function Bool isStrideClass(Bit#(16) pcHash);
        let entry = strideClassifier[strideIdx(pcHash)];
        Bit#(3) thresh = fromInteger(valueOf(strideClassThreshold));
        return entry.valid
            && entry.pcHash == pcHash
            && entry.matchCount >= thresh
            && entry.stride != 0;
    endfunction
    ...
    // Classifier maintenance: drain (pcHash, lineAddr) from strideUpdateQ and
    // update per-PC state. Combinational read/write of register file.
    rule strideUpdate(inited);
        match {.pcHash, .lineAddr} = strideUpdateQ.first;
        strideUpdateQ.deq;
        Bit#(6) idx = strideIdx(pcHash);
        let cur = strideClassifier[idx];
        Bit#(34) newLineAddr = truncate(pack(lineAddr));
        StrideClassEntry2 next;
        if (!cur.valid || cur.pcHash != pcHash) begin
            // New PC for this slot -- reset state
            next = StrideClassEntry2{
                valid: True, pcHash: pcHash,
                lastLineAddr: newLineAddr, stride: 0, matchCount: 0
            };
        end else begin
            Int#(13) observedStride = truncate(unpack(pack(newLineAddr) - pack(cur.lastLineAddr)));
            if (observedStride == cur.stride && cur.stride != 0) begin
                Bit#(3) mc = (cur.matchCount == maxBound) ? maxBound : cur.matchCount + 1;
                next = StrideClassEntry2{
                    valid: True, pcHash: pcHash,
                    lastLineAddr: newLineAddr, stride: cur.stride, matchCount: mc
                };
                $display("%t AlexLog: CDP Hybrid2 stride match pcHash %h stride %d matchCount %d -> %d",
                    cur_cycle, pcHash, cur.stride, cur.matchCount, mc);
            end else begin
                next = StrideClassEntry2{
                    valid: True, pcHash: pcHash,
                    lastLineAddr: newLineAddr, stride: observedStride, matchCount: 0
                };
            end
        end
        strideClassifier[idx] <= next;
    endrule
        ...
        case (tag) matches
            tagged Training {.pcHash, .relOffset}: begin
                ...
                if (rdResp matches tagged Valid .e) begin
                    if (e.pcHash != pcHash) collision = True;
                    else curConf = e.conf;
                end
            ...
            tagged PrefetchIssue {.addr, .line, .reqVpn}: begin
                if (rdResp matches tagged Valid .entry) begin
                    ...
                    if (foundHighConf) begin
                        // Classifier dispatch:
                        //   STRIDE-class  -> emit stride-ahead prefetch (CDP content-scan replaced)
                        //   non-STRIDE    -> emit CDP content-scan prefetch (unchanged from L)
                        Bool isStrided = isStrideClass(entry.pcHash);
                        Bit#(4) bestIdx = pack(bestRelOffset + 7);
                        $display("%t AlexLog: CDP Hybrid2 prefetch decision: pcHash %h relOffset %d conf %d %s",
                            cur_cycle, entry.pcHash, bestRelOffset, entry.conf[bestIdx],
                            isStrided ? "STRIDE_EMIT" : "CDP_EMIT");
                        if (isStrided) begin
                            // Stride prefetch: currentAddr + stride(lines) * 64
                            StrideClassEntry2 ce = strideClassifier[strideIdx(entry.pcHash)];
                            Int#(19) strideBytes = unpack({pack(ce.stride), 6'b0});
                            Addr strideOffset = pack(signExtend(strideBytes));
                            Addr strideTarget = addr + strideOffset;
                            Bool crossPage = getVpn(strideTarget) != reqVpn;
                            tlbReqFIFO.enq(tuple3(strideTarget, False, crossPage));
                            $display("%t AlexLog: CDP Hybrid2 stride emit: pcHash %h stride %d target %h crossPage %b",
                                cur_cycle, entry.pcHash, ce.stride, strideTarget, crossPage);
                        end else begin
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .storedAddr &&& storedAddr == lineAddr) begin
            $display("%t AlexLog: CDP Hybrid2 filter HIT: dropped duplicate prefetch for lineAddr %h", cur_cycle, lineAddr);
        end else begin
    ...
    method Action reportIncomingCacheLine(reqT req, Line line, Bool cRqIsPrefetch, Bool wasMiss, Bool wasNeighbourPrefetch);
        if (inited && getReqOp(req) == Ld && !cRqIsPrefetch && wasMiss && !wasNeighbourPrefetch) begin
            ...
            l1ToCDP.enq(tmp);
            // Classifier update: demand Ld miss
            strideUpdateQ.enq(tuple2(getPcHash(req), getLineAddr(getReqAddr(req))));
            ...
            // Classifier update: demand Ld hit
            strideUpdateQ.enq(tuple2(getPcHash(req), getLineAddr(getReqAddr(req))));
