// CDPBMAlign8Supp.bsv -- BMPolySupp + pointer-shape filters.
...
typedef struct { LineAddr lineAddr; } BMAlign8SuppFilterEntryT deriving (Bits, FShow, Eq);
...
provisos (
    ...
    RWBramCore#(Bit#(10), Maybe#(BMAlign8SuppFilterEntryT)) prefetchFilter <- mkRWBramCoreForwarded();
    ...
    function Bool isPtrShape(Addr w);
        ...
        Bool aligned8  = (w[2:0] == 0);
        Bool nonNull   = (w != 0);
        return upperZero && aligned8 && nonNull;
    endfunction
        ...
        if (!suppress) begin
            for (Integer i = 0; i < 8; i = i + 1) begin
                ...
                if (candUpper == missUpper && isPtrShape(x.line[i])) begin
                    ...
                    $display("%t AlexLog: CDP BMAlign8Supp candidate offset: %d candVaddr: %h matchCount: %d L1",
                        cur_cycle, i, x.line[i], matchCount);
                    enqIdx = enqIdx + 1;
                end
            end
        end else begin
            $display("%t AlexLog: CDP BMAlign8Supp SUPPRESS matchCount: %d (multi-match)",
    ...
    rule processTlbResp(filterInited && tlbReqFreeQInited);
        ...
        if (!resp.haveException && paddrInMainMem) begin
            ...
            $display("%t AlexLog: CDP BMAlign8Supp dropped vaddr %h paddr %h (exc=%b mainMem=%b)",
    ...
    rule processFilterResp(filterInited);
        ...
        if (rdResp matches tagged Valid .fe &&& fe.lineAddr == lineAddr) begin
            $display("%t AlexLog: CDP BMAlign8Supp filter HIT, dropping lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(BMAlign8SuppFilterEntryT{ lineAddr: lineAddr }));
            nextCandidateBuffer.enq(NextCandAlignSuppT{paddr: paddr, vaddr: candVaddr, routeLLC: routeLLC});
            $display("%t AlexLog: CDP BMAlign8Supp filter MISS, issuing prefetch lineAddr %h routeLLC: %b", cur_cycle, lineAddr, routeLLC);
        end
    endrule
