// CDPLLC.bsv — Variant B: LLC-routed CDP stateful relative prefetcher.
//
// Identical to mkCDPStatefulRelative except that getNextPrefetchAddr sets
// PendingPrefetch.nextLevel = True. L1Bank routes nextLevel=True prefetches
// directly to the parent (LLC) without creating an L1 MSHR entry — i.e., the
// prefetch warms the LLC but does NOT populate L1 (zero L1 pollution).
//
// Rationale: profiling on 2026-04-20 revealed baseline CDP is 5-28% SLOWER
// than no prefetcher on 4/5 pointer-chasing benchmarks because imprecise
// L1 prefetches evict useful demand lines. LLC-routing trades L1-hit latency
// for LLC-hit latency on the prefetched line, while leaving L1 untouched.
//
// Best for workloads where CDP's hit-rate is low enough that pollution
// dominates the speedup from useful prefetches — in our suite, everything
// except patricia (which loses speedup without L1-resident prefetches).
...
import Cur_Cycle::*;
import CDP::*; // for shared types: NextCandT, RelLineOffset, TrainingTableEntryT,
               // TrainingTableRespQT, PCRelOffsetConfT, PCTableEntryT, PCTableRdRelTagT

module mkCDPStatefulRelativeLLC#(
...
provisos (
    ...
    rule doTrainingTableInit(!ttInited);
        ...
        if (ttInitCount == maxBound) begin
            ttInited <= True;
            $display("%t AlexLog: CDP LLC Training table inited", cur_cycle);
        end
    ...
    rule doPcTableInit(!pcInited);
        ...
        if (pcInitCount == ~0) begin
            ...
            $display("%t AlexLog: CDP LLC PC table inited", cur_cycle);
        end
    ...
    rule doFilterInit(!filterInited);
        ...
        if (filterInitCount == ~0) begin
            filterInited <= True;
            $display("%t AlexLog: CDP LLC prefetch filter inited", cur_cycle);
        end
    ...
    rule doTlbReqFreeQInit(!tlbReqFreeQInited);
        ...
        if (tlbReqFreeQInitCount == ~0) begin
            ...
            $display("%t AlexLog: CDP LLC TLB free queue inited", cur_cycle);
        end
    ...
    rule deqCacheLines;
        ...
        $display("%t AlexLog: CDP LLC deqCacheLines", cur_cycle);
        if (getReqOp(x.req) == Ld) begin
            ...
            for (Integer i = 0; i < 8; i = i + 1) begin
                ...
                if (candUpper == missUpper) begin
                    ...
                    $display("%t AlexLog: CDP LLC candidate vaddr relOffset: %d pcHash: %h candVaddr: %h crossPage: %b",
    ...
    rule ttAccess(inited);
        ...
        if (respQ.isTrainingLookup) begin
            if (rdResp matches tagged Valid {.hitWay, .ttRdResp}) begin
                ...
                $display("%t AlexLog: CDP LLC Training hit: missVaddr %h seen before by pcHash %h at relOffset %d",
            ...
            if (rdResp matches tagged Valid {.hitWay, .ttRdResp}) begin
                ...
                $display("%t AlexLog: CDP LLC Overwrote to training table, idx: %d candVaddr: %h relOffset: %d",
                    cur_cycle, respQ.ttIdx, respQ.candVaddr, relOffset);
            end else begin
                trainingTable.wrReq(respQ.ttIdx, rdRepl, newEntry);
                $display("%t AlexLog: CDP LLC Wrote to training table, idx: %d candVaddr: %h relOffset: %d",
                    cur_cycle, respQ.ttIdx, respQ.candVaddr, relOffset);
            end
        end
    endrule
        ...
        case (tag) matches
            tagged Training {.pcHash, .relOffset}: begin
                ...
                if (rdResp matches tagged Valid .e) begin
                    if (e.pcHash != pcHash)
                        $display("%t AlexLog: CDP LLC PC table collision at idx: %d evicted pcHash: %h new pcHash: %h",
                ...
                $display("%t AlexLog: CDP LLC PC table updated, idx: %d pcHash: %h relOffset: %d conf: %d -> %d",
            ...
            tagged PrefetchIssue {.addr, .line, .reqVpn}: begin
                if (rdResp matches tagged Valid .entry) begin
                    ...
                    if (!foundHighConf) begin
                        ...
                        $display("%t AlexLog: CDP LLC no high-conf offset: pcHash %h maxConf %d",
                            cur_cycle, entry.pcHash, maxConf);
                    end
                    if (foundHighConf) begin
                        Bit#(4) bestIdx = pack(bestRelOffset + 7);
                        Bool isNeighbour = !(bestAbsTarget >= 0 &&& bestAbsTarget <= 7);
                        $display("%t AlexLog: CDP LLC prefetch decision: pcHash %h relOffset %d conf %d isNeighbour %b",
                        ...
                        if (bestAbsTarget >= 0 &&& bestAbsTarget <= 7) begin
                            ...
                            if (candUpper == addrUpper) begin
                                ...
                                $display("%0d AlexLog: CDP LLC queued TLB req for candidate vaddr %h pcHash %h relOffset %d",
            ...
            tagged Decay: begin
                if (rdResp matches tagged Valid .entry) begin
                    ...
                    $display("%t AlexLog: CDP LLC decay applied to pcTable idx: %d pcHash: %h", cur_cycle, pctIdx, entry.pcHash);
                end
            end
        endcase
    endrule
    ...
    rule processFilterResp(inited);
        ...
        if (rdResp matches tagged Valid .storedAddr &&& storedAddr == lineAddr) begin
            $display("%t AlexLog: CDP LLC filter HIT: dropped duplicate prefetch for lineAddr %h", cur_cycle, lineAddr);
        end else begin
            prefetchFilter.wrReq(filterIdx, Valid(lineAddr));
            nextCandidateBuffer.enq(cand);
            $display("%t AlexLog: CDP LLC filter MISS: issuing prefetch for lineAddr %h", cur_cycle, lineAddr);
        end
    endrule
    ...
    // ** KEY DIFFERENCE vs mkCDPStatefulRelative: nextLevel = True (LLC-routed). **
    method ActionValue#(PendingPrefetch) getNextPrefetchAddr;
        let x = nextCandidateBuffer.first;
        nextCandidateBuffer.deq;
        $display("%t AlexLog: CDP LLC Prefetch addr issued (LLC-routed). lineAddr: %h", cur_cycle, getLineAddr(x.paddr));
        return PendingPrefetch {
            addr: x.paddr,
            vpn: getVpn(x.vaddr),
            nextLevel: True,             // <-- the difference: LLC-only, no L1 pollution
